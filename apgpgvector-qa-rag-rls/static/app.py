...
# List of users for whom the documents will be uploaded
user_list = ('All','user1','user2','user3')


# This is Custom retriver class enables to add context to the query from the Aurora PostgreSQL table 
...
# Based on the username, it sets the session role to enfoce the Row Level Security in Aurora PostgreSQL table 
def similarity_search(query):
    ...
    if 'username' in st.session_state:
        if st.session_state.username != 'All':
            dbconn.execute("set role={}".format(st.session_state.username))
...
# This function takes the text chunks and username as input and creates a vector store using pgvector.
...
def store_vector(username,chunks):
    ...
    ee = embeddings.embed_documents(chunks)

    for idx, x in enumerate(chunks):
        dbconn.execute("""INSERT INTO qa_rag_rls (username, document, embedding)
                          VALUES(%s, %s, %s);""", (username, x, ee[idx]))
    dbconn.close()


# Here, a conversation chain is created using the conversational AI model 
# This chain allows the Gen AI app to engage in q&A interactions.
def get_qa_chain():

    # Define model_id, client and model keyword arguments for FLAN T5
    ...
    # When prompting LLM through the API, it is very important to use the correct formatting.
    # LLM's are trained as a conversational agent using these special tokens to mark who is speaking.
    prompt_template = """Use the following pieces of context to answer the question at the end. If you don't know the answer, just say that you don't know, don't try to make up an answer. Use three sentences maximum with bullet-points. Keep the answer as concise as possible. Always say "thanks for asking!" at the end of the answer. 
{context}
Question: {question}
Helpful Answer:"""
...
def handle_userinput(user_question, username):
    ...
    chat_history.append({
                        "query" : "{} ({})".format(response['query'], username),
...
# Function responsible for writing the chat history to the console
...
def main():
    ...
    st.set_page_config(page_title="RAG-based Q&A with Aurora PostgreSQL pgvector and RLS",
                       layout="wide",
    ...
    st.header("RAG-based Q&A with Aurora PostgreSQL pgvector and RLS")
    subheader = '<p style="font-family:Calibri (Body); color:Grey; font-size: 16px;">Leverage Foundational Models and <a href="https://github.com/pgvector/pgvector">pgvector</a> as Vector Engine with PostgreSQL Row level security</p>'
    ...
    st.image(image, caption='Generative AI Q&A with Aurora PostgreSQL and pgvector')
